// src/app/app/page.tsx
'use client';

// 1. Import all the providers
import { ConnectionProvider } from '@/context/ConnectionContext';
import { SeasonDataProvider } from '@/context/SeasonDataContext';
import { UserHoldingsProvider } from '@/context/UserHoldingsContext';
import { AuctionProvider } from '@/context/AuctionContext';
import { ExchangeProvider } from '@/context/ExchangeContext'; // Ensure this is imported

// 2. Import all the display components
import { ConnectionDisplay } from '@/components/ConnectionDisplay';
import { SeasonDisplay } from '@/components/SeasonDisplay';
import { UserHoldingsDisplay } from '@/components/UserHoldingsDisplay';
import { AuctionForm } from '@/components/AuctionForm';
import { Exchange } from '@/components/Exchange';

export default function DAppPage() {
  return (
    // 3. Nest all providers at the top level. The order of providers
    // that don't depend on each other doesn't matter, but it's good practice
    // to have them all here, wrapping the entire page.
    <ConnectionProvider>
      <SeasonDataProvider>
        <UserHoldingsProvider>
          <AuctionProvider>
            <ExchangeProvider>
              <main className="flex min-h-screen flex-col items-center p-8 md:p-24 bg-bg">
                <div className="w-full max-w-4xl text-center space-y-8">
                  <h1 className="text-4xl font-bold">FIM Token Auction & Exchange</h1>
                  
                  {/* 4. All of these components are now guaranteed to be children of ALL providers */}
                  <ConnectionDisplay />
                  <AuctionForm />
                  <UserHoldingsDisplay />
                  <Exchange />
                  <SeasonDisplay />

                </div>
              </main>
            </ExchangeProvider>
          </AuctionProvider>
        </UserHoldingsProvider>
      </SeasonDataProvider>
    </ConnectionProvider>
  );
}