  ...
  const { bids, asks } = useMemo(() => {
    ...
    const allOrders: Order[] = orderResults
      ...
      .map(o => {
        ...
        let price = 0;
        if (orderType === OrderType.BID) price = parseFloat(formatUnits(amountToBuy, 18)) / parseFloat(formatUnits(amountToSell, 6));
        else price = parseFloat(formatUnits(amountToBuy, 6)) / parseFloat(formatUnits(amountToSell, 18));
        return { id, creator, orderType, amountToSell, amountToBuy, amountFilled, price, amountRemaining: amountToSell - amountFilled };
      });
