...
The collateral rule has one honest limit, and the protocol addresses it head-on rather than pretending otherwise. Because the lock is proportional to *holdings*, it prices what a wallet owns, not how many wallets there are: a whale who splits an existing hoard across two hundred accounts locks exactly the same total collateral as if it sat in one. Building fake wallets from nothing is expensive; splitting real wealth into fake "small holders" is not. Crucially, *no per-wallet fee can close this gap* — an honest player pays it once and a whale pays it N times, but any amount low enough not to exclude a poor honest player is one a whale pays N times trivially. So the protocol does not try to price headcount at all. Instead it catches the manipulation and voids its result: a season's decided outcome passes through a review period (§9.5) in which the Council, executing a DAO vote, can flag a wallet-cluster as a splitting attack — whereupon the flagged wallets receive nothing and the whole season settles as a draw, neutralizing the manufactured result. No RGD is ever burned or seized; a flag costs the attacker their winnings, and the collateral — everyone's, flagged or not — is untouchable and always comes back.

The effect is accountability. No one can move the game's state from behind a wall of disposable wallets; influence requires a real, lasting stake in the protocol—and pretending to be many people puts real capital at risk for every identity claimed.
...
**Before the money moves: the review period.** A decided outcome does not open payouts immediately. It first passes through a two-stage review. In a short **triage** window (a per-season setting, proposed at 24 hours) the Execution Council may raise a suspicion; if it does not, distribution opens on the outcome. A suspicion opens a longer **investigation** window (proposed at 14 days) in which the Council—executing a DAO vote—may **flag** wallets it judges to be a manufactured sybil cluster: a split hoard posing as a crowd, or a wash-traded contribution profile. The sanction is deliberately narrow and known in advance. Because a flag asserts the *outcome itself* was manufactured, any single flag makes the whole season settle as a **draw**—pro-rata of final holdings, a distribution that takes no side—and flagged wallets receive **nothing**. No RGD is burned or seized: a flag costs the attacker their winnings, never their staked collateral or principal. And review can never be abused to stall: when either window lapses, **anyone** may open the payout phase (the Council can also conclude an investigation early), so an unresolved review always resolves to *pay everyone*—the Council's inaction can never freeze anyone's money.

**After the money waits: the Claim Window.** Payouts stay claimable for a full year after the payout phase opens. Whatever remains unclaimed after that may be swept by the Council—the same end-of-life rule the testnet reward distributor uses, and standard practice for expired airdrops. A sweep ends only the *prize*: a player who shows up even later can still complete their claim, receiving zero payout but recovering their staked collateral in full. A player's own capital is recoverable forever; only the winnings expire.
...
A fixed token amount makes the cost of participation deterministic and known in advance, immune to price manipulation. One limit is acknowledged openly: because the lock scales with *FIM held*, it prices holdings, not headcount—an attacker splitting an existing hoard across many wallets locks the same total either way. That residual attack surface is not left to the collateral rule; it is closed by a separate, headcount-priced mechanism (D8).
...
The bond exists to defend a specific attack. Settlement processes the player set in batches; if anyone could submit batches, a griefer could interleave a poisoned or out-of-order batch into a partially-processed settlement and stall it in an unfinalisable state, freezing everyone's funds. So once a starter has bonded, **only they** may submit that season's batches, and the batches are validated (sorted, de-duplicated, dust-filtered) on the way in. An honest finalisation returns the bond; an attempt to settle before a victory or the time limit is actually met forfeits the bond to the DAO, which makes spamming premature settlements costly. Should a bonded starter abandon a batch mid-flight, the Execution Council can reassign the crank via `resetSettlement` — but neither the starter nor the Council is a liveness dependency. Every settlement round carries a hard deadline (`settlementTimeoutSeconds`, proposed at 24 hours): once missed, **anyone** may seize the starter role via `takeOverSettlement` by posting a fresh bond, with the abandoned bond forfeited to the season's DAO recipient. Stalling therefore costs a bond per timeout period, and a frozen season always self-heals within one deadline — settlement liveness depends on no privileged party at all. The net effect: settlement is open to anyone, cheap for an honest starter, and expensive to grief.
...
This makes the cost of finalisation independent of how many players joined, removing the population-based DoS entirely. It also hardens a subtler property: because `claimPayout` always releases the player's RGD collateral even when the payout is zero—or when the wallet was flagged—no player's stake can be stranded by a season they technically "lost"—everyone can always recover their locked collateral regardless of outcome. The same reasoning eliminated end-of-season order cleanup altogether: the season's ledger only moves on fills, so FIM escrowed in an open order never stops counting as its owner's balance, payouts are fully independent of open orders, and finalisation performs zero Exchange work. Owners reclaim their own escrow via `cancelOrder` (or the batch `cancelOrders`), at any time, forever—there is no drain to crank and no keeper to depend on.

**D7 — FIM can only move through the Exchange, not by free transfer.**
FIM is a restricted ERC-20: its `transfer`/`transferFrom` are gated so that only the season's `Exchange` can move tokens, and minting is restricted to the `Auction`. A normal freely-transferable token is rejected here for a precise reason: if players could send FIM wallet-to-wallet outside the order book, they could rearrange the wealth distribution—and therefore the Gini score the whole game is measured by—through private transfers the market never sees.

Forcing every movement of FIM through the observable Exchange guarantees that each shift in wealth is recorded on-chain, priced, and subject to the trade fee, so the Gini metric always reflects the true distribution and cannot be quietly manipulated off-book. It also means the counterparty-selection mechanic (D1) cannot be bypassed: there is no side door around the transparent market. The cost of this choice is that FIM is unusable in any external venue or composable DeFi context, which is acceptable because FIM is a disposable, season-local game token with no purpose outside the game. There is deliberately no burn function: once a season ends, its FIM is inert by construction (minting is auction-only, fills require an active season), and payouts read the season's ledger rather than the token, so destroying leftover tokens would be pure cosmetics.

**D8 — Sybil defense by detection and outcome-nullification, not per-wallet fees or confiscation.**
D4 prices holdings; something must answer *headcount*, because splitting a hoard across wallets is collateral-neutral and no statistic over anonymous balances can detect it (splitting is observationally identical to honest dispersion). The decisive realisation is that *no per-wallet economic lever can do this job*: an honest player pays any per-wallet cost once, an attacker pays it N times — but the same cost multiplies both, and the constraint "small enough not to exclude poor players" caps it at a value a whale pays N times trivially. A flat bond, a refundable minimum stake, and the existential threshold are all this same limited lever (the threshold merely fixes the minimum wallet size the attacker then builds on). So none is asked to carry deterrence; the protocol charges no sybil-specific per-wallet fee at all. Collateral is likewise never made slashable to fill the gap: it is the continuously-maintained backing of a freely-trading position, shared across seasons, so burning it would un-back live FIM and strip backing from sibling seasons — and slashing it on one season's fallible, unfalsifiable-evidence flag would punish exactly the honest multi-season player while missing the fresh-wallet attacker.

What deters instead is the thing that scales with what the attacker actually buys — a manufactured *outcome* — namely catching it and voiding it. Each decided outcome passes through a two-stage review before payouts open. In a short **triage** window the Council may raise a suspicion; if it does not, distribution opens on the outcome. A suspicion opens a longer **investigation** window in which the Council, executing a DAO vote, may flag wallet-clusters. Because a flag asserts the *outcome itself* was manufactured, any single flag makes the whole season settle as a **draw** (pro-rata of final holdings, a distribution that takes no side), and flagged wallets receive **zero** — never touching any player's collateral or principal. Both windows are deadman-bounded: when either lapses, anyone opens distribution, so review can delay payouts by at most triage+investigation and an unresolved investigation always resolves to *pay everyone*.

The rejected extremes frame why this is the shape. Identity verification (proof-of-personhood, wallet-history scoring) was rejected as the foundation: the EVM cannot read wallet histories, off-chain scoring imports an oracle dependency, and industrial airdrop farms defeat such heuristics routinely. Full confiscation of a flagged wallet's stake was rejected even more firmly: it would let governance seize player capital on unfalsifiable evidence, give every other holder a motive to support flagging any large winner, and put total-loss tail risk on exactly the deep-capital players the game needs (the Solend episode of 2022 — a DAO voting, with sincere intentions, to seize a whale's account — is the precedent). The accepted cost of the shipped design is honestly twofold: a wrongful flag flattens the *whole* season to a draw (every honest winner loses winning-side upside, though never principal or collateral, and falls back to a fair pro-rata), and deterrence is now post-hoc — an attack the Council never notices pays off, with only the attacker's own RGD-price exposure as a diffuse, evadable backstop. Both are the price of refusing every per-wallet fee and every confiscation power; parameters are immutable within a season but tunable between them (graduated response).

A flag's effect is confined to the season in which it is raised. Cross-season *exclusion* — barring a flagged wallet from future seasons — was rejected: wallets are free, so it cannot touch an attacker (who funds fresh addresses), while it converts the bounded false-positive cost into an open-ended one borne only by honest players with persistent identities, and it would demand a permanent, Council-writable participation blacklist whose worst case (gatekeeping who may *enter*) exceeds any in-season sanction. Flags are permanent public data, so a previously-flagged wallet can be *displayed* as such in later seasons — reputation by information, not by protocol-enforced law.
...
- ***Multi-Ledger Accounting:*** It tracks `seasonPrincipals` per season alongside a `totalGlobalPrincipal`, ensuring the principal owed to one season is never paid out to another. With multiple seasons live at once, **every** harvest attributes the *entire* realized surplus across all accruing seasons, pro-rata by principal, into per-season `accruedYield` buckets — so no yield is ever left unattributed and a late-harvesting season receives exactly its share rather than a residual-diluted fraction. A season retires from the accrual set at its own (single) harvest; whatever its parked claim reserve earns afterwards flows to the seasons still accruing. A season can never drain the yield earned on another season's capital.

- **Season-Locked Policies:** Each season's revenue policy (Buyback / Liquidity / Prize-Pool-reinvest / DAO basis points) is snapshotted at deployment via `setSeasonPolicy` and must sum to 10 000. The policy also carries the season's **revenue recipient addresses** (buyback, liquidity, DAO): recipients are per-season published terms, fixed at creation and visible on-chain before any player joins — not mutable protocol globals. The Treasury deliberately exposes **no recipient setters at all**; rotating a recipient means updating the manifest for the *next* season, never changing anything under a live one. `harvestAndExecutePolicy` applies the splits **to that season's realized yield only** — never to principal: the DAO and liquidity shares are transferred to the season's recipients, the prize-pool share is reinvested by adding it back to the season's principal, and the buyback share is forwarded as USDC to the season's `buybackRecipient`. The Treasury forwards that share for the DAO-controlled recipient to execute separately, rather than performing an automatic on-chain buy-and-burn of \$RGD—a design that avoids an exploit vector.
...
- **Unclaimed-Principal Sweep:** A season-gated `sweepSeasonPrincipal(to)` lets a season sweep **only its own** residual principal (withdrawing from Aave if needed). It is reachable exclusively through `GameSeason.sweepUnclaimed` after the 365-day Claim Window elapses — the timing guards live in the season contract itself, and not even the Treasury owner can trigger a sweep directly.
...
- **Ledger Sync:** Every fill calls `GameSeason.updateLedger` so the season's authoritative \$FIM balances and net-contribution figures stay consistent with on-chain settlement. There is deliberately **no end-of-season order drain** (see D6): escrow never leaves its owner economically, season finalization performs zero Exchange work, and owners reclaim their escrow themselves via `cancelOrder`/`cancelOrders`, at any time, forever.

**`GameSeason.sol` (The State Machine & Arbiter)**

- **Function:** The autonomous referee managing season state and settlement.

- **The Phase Engine:** An internal `State` enum surfaces to clients through `getPhase()` as the human-readable lifecycle `AUCTION → BOOTSTRAP → TRADING → SETTLING → REVIEW → PAYOUT`. `getPhase()` returns the distinct string `AUCTION` while the capital-formation window is open; once it closes, the season reports `BOOTSTRAP` until `G_initial` is established, then `TRADING`. The internal `CALCULATING`/`REVIEW`/`DISTRIBUTION` states surface as `SETTLING`/`REVIEW`/`PAYOUT`. The season remains in `DISTRIBUTION` (`PAYOUT`) indefinitely so latecomers can always claim.
...
- **Trustless Verification (The Arbitrator):** Any external actor (a Keeper or Bot) drives a state transition by posting a USDC bond (`bondAmountUsdc`), which makes them the season's `settlementStarter`. **Only that bonded starter may submit batches** via `processBatch`, and the set must be in **ascending balance order**; the validation logic enforces a sort check, a no-duplicates check, and the dust filter (`existentialThresholdFim`). Gating batch submission to the bond-poster prevents a griefer from poisoning a partially-processed batch into an unfinalisable state; should a starter abandon a batch mid-flight, the Execution Council can reassign it with `resetSettlement` — and once the round's hard deadline (`settlementTimeoutSeconds`) lapses, **anyone** may seize the role via `takeOverSettlement` by posting a fresh bond, with the abandoned bond forfeited to the season's DAO recipient (see D5). On a successful finalization the bond is returned to the starter; an attempted settlement that does not yet meet a victory or time condition forfeits the bond **to the DAO** (routed to the season's DAO recipient, not the Treasury balance, so it is never miscounted as yield).
...
- **Settlement Engine:** On a triggering condition `finalizeGame` locks the outcome and enters `TRIAGE`. During triage the Council may `raiseSuspicion` to escalate to `INVESTIGATION`, where it (executing a DAO vote) may `flagWallets`; a flagged wallet gets zero and any single flag forces the whole season to settle as a draw — never touching collateral (see D8). When either review window lapses, **anyone** may call `openDistribution` (the Council can also `concludeInvestigation` early), which harvests Treasury yield, snapshots the final prize pool (`finalPoolSize`), and moves to `DISTRIBUTION`. Payouts are computed **lazily, per player, at claim time** (`claimPayout`), so finalization cost is independent of the player count. `claimPayout` **always releases the player's \$RGD collateral — even on a zero payout, even when flagged** — so no player's stake can ever be stranded by an ended season; USDC is transferred only when a payout is owed. Claims are fully independent of the Exchange: the ledger balance already includes any \$FIM still escrowed in open orders, and no token burn is performed.

- **Payout Preview:** The view function `computePayout(player)` returns exactly what a player would receive from `claimPayout` right now, enabling frontends to display pending rewards without requiring a transaction. It returns 0 before distribution opens (including during triage and investigation), after a sweep, for already-claimed players, for flagged wallets, and for players below the existential threshold; when any flag has forced a draw it returns the player's draw-rate share.

- **Unclaimed-Principal Sweep:** If any USDC principal remains unclaimed 365 days after the season enters `DISTRIBUTION` (the Claim Window), the owner (Execution Council) may call `sweepUnclaimed(to)`, which forwards the season's residual principal from the `Treasury` to the specified address. The sweep is once-only and ends only the *prize*: post-sweep claims still succeed at a zero payout and release the claimant's collateral in full. This 1-year lockout gives every participant ample time to claim before any administrative recovery is possible.

**`FIM.sol` (The Seasonal Asset)**

- **Function:** A restricted ERC-20. `mint` is callable only by the season's `Auction`, and `transfer`/`transferFrom` are gated so that only the `Exchange` can move tokens — forcing every value transfer through the observable order book. There is no burn function: post-season \$FIM is inert by construction, and payouts read the season ledger rather than the token (see D7).
...
1. *Trigger:* The bonded starter (`settlementStarter`) posts the USDC bond via `startSettlement()` (the same bonded-crank mechanism used by `startBootstrap()` to establish `G_initial`), submits the sorted player set through `processBatch()`, then calls `finalizeGame()`. If a starter stalls past the settlement deadline, anyone may seize the role via `takeOverSettlement()`.

2. Review: If a victory or the time limit is met, the outcome is locked (`CALCULATING → TRIAGE`) and enters two-stage sybil review — the Council may escalate to an investigation and flag wallet-clusters, any flag settling the season as a draw (D8).

3. Distribution Opening: once the window lapses, anyone calls `openDistribution()`: `harvestAndExecutePolicy` realizes the season's attributed Aave yield and applies the revenue policy, the final prize pool is snapshotted (`finalPoolSize` from `getSeasonPoolSize`), and the state moves to `DISTRIBUTION`.

4. Settlement Calculation: individual payouts are computed lazily, per player, when each calls `claimPayout` — which also returns the player's collateral. Open-order escrow is reclaimed by each owner via `cancelOrders` at their leisure.

5. Incentivisation: The starter's bond is returned on a successful finalization (a premature settlement attempt instead forfeits the bond to the DAO, discouraging spam; an abandoned round forfeits it to the DAO at takeover).

**The Operational Lifecycle:** Auction → Bootstrap → Trading → Settling → Review → Payout
...
**The Uniswap V2 pool that `finalize()` creates (§13.1) opens the secondary market at parity with the auction.** Because the sale tranche and the liquidity tranche are the same size—5% each—the pool's opening price is exactly the clearing price the auction discovered, so there is no gap between the auction price and the first day of trading. To protect the genesis listing from front-running, `finalize()` refuses to deposit if the \$RGD/USDC pair has been pre-created and seeded with skewed reserves, and supplies liquidity with non-zero minimum-amount bounds rather than accepting any ratio; an owner-only `recoverToken` escape hatch (disabled once finalized, and **barred from ever touching deposited USDC**) allows recovery of the \$RGD if such a griefing attempt blocks the listing. As a circuit breaker, the owner can irreversibly `abortAuction()` if a security flaw is discovered during the ILO: finalization and deposits are voided forever, and every depositor reclaims their own USDC via a self-service `refund()` with no deadline. The owner's emergency power is strictly "stop and give everyone their money back" — deposits have exactly two exits, both trustless: `finalize()` into the liquidity pool per the published rules, or `refund()` to their owner.
...
- **The Mitigation:** Detection and outcome-nullification, not a per-wallet fee — because no per-wallet economic lever can distinguish one honest wallet from N attacker wallets (both pay any such cost, and the amount must stay low enough for a poor honest player). Two facts do the work:

- **Holdings cost real collateral, but headcount is not priced.** A bottom-up sybil building minion wallets from nothing must fund every one and lock RGD behind every unit of FIM — but a whale splitting an *existing* hoard locks the identical total whether it sits in one wallet or two hundred. The protocol does not try to tax the splitting with a fee (any fee small enough for the poor is one the whale pays N times trivially); it targets the *outcome* the split is trying to buy.

- **Outcomes are reviewable, and a flag voids the outcome.** A decided result passes through a two-stage review (triage, then investigation) before payouts open. If the Council — executing a DAO vote — flags a wallet-cluster as a split, the flagged wallets receive nothing and the whole season settles as a draw, so the manufactured victory simply evaporates. No RGD is burned or seized; the sanction is forfeited winnings, never principal or collateral. Because a well-executed split looks just like honest dispersion on-chain, this bounded human check is the deterrent that scales with the payoff the attacker is chasing. The accepted costs are stated plainly (D8): a wrongful flag flattens the whole season to a draw, and an attack the Council never notices pays off, with the attacker's own RGD-price exposure as the only automatic — and diffuse, evadable — backstop.
...
**Triage:** The short window (default 24h) after a season's outcome is decided, in which the Execution Council may raise a suspicion to escalate to an Investigation. If it does not, distribution opens on the outcome.

**Investigation:** The longer window (default 14 days) a suspicion opens, in which the Council (executing a DAO vote) may flag sybil wallet-clusters. It always resolves to payouts when it lapses — flags or not — and can be concluded early; it can never freeze payouts.

**Flag:** The Council marking a wallet as a sybil participant during an Investigation. The flagged wallet receives zero payout, and because a flag asserts the outcome was manufactured, any single flag makes the whole season settle as a draw. Never touches collateral or principal; no \$RGD is burned.

**Claim Window:** The 365 days after distribution opens during which payouts are claimable. Unclaimed residuals may then be swept, but a late claim still succeeds at zero payout and releases the player's collateral — a player's own capital is recoverable forever.
